...
impl Dial1 {
    pub fn do_instruction(&mut self,inst:&Instruction) {
        ...
        match inst.direction() {
            Direction::L => self.current = (self.current + self.n_numbers - num)%self.n_numbers,
            Direction::R => self.current = (self.current + num)%self.n_numbers,
        }
...
pub fn part1(instructions:&Vec<Instruction>) {

    let mut dial = Dial1::new();
